...
function Field({ label, id, defaultValue, readOnly = false }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <input
        ...
        readOnly={readOnly}
        className={`h-[34px] w-full rounded-[6px] border border-white/20 bg-night px-3 text-[12px] text-white outline-none ${
          readOnly ? 'opacity-60' : 'focus:border-accent'
        }`}
      />
    </div>
  )
...
const ACCOUNT = {
  user: { name: 'Deniz Kızılbora', email: 'deniz@sports.com' },
  agent: { name: 'Denizz K.', email: 'agent@sports.com' },
  admin: { name: 'Denizzz K.', email: 'admin@sports.com' },
}
...
export default function ProfileSettings({ role = 'user' }) {
  ...
  return (
    <AppLayout title="Profile & Settings" role={role}>
      <div className="flex flex-wrap items-start gap-8 p-7">
        <div className="flex min-w-[520px] flex-[1.6] flex-col gap-6">
          <section className="flex flex-col gap-6 rounded-[15px] border border-white/10 bg-card p-8">
            <div className="flex items-start gap-7">
              ...
              <div className="flex flex-col gap-2 pt-2">
                <p className="text-[24px] font-bold">{account.name}</p>
                <p className="text-[14px] text-white/80">Account created: 01/07/2026</p>
                {saved && <p className="text-[12px] font-bold text-accent">Profile updated ✓</p>}
              </div>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-5">
              <Field label="Full Name" id="p-name" defaultValue={account.name} readOnly={!editing} />
              <Field label="Email" id="p-email" defaultValue={account.email} readOnly={!editing} />
              <div />
              <Field label="Phone" id="p-phone" defaultValue="+90 5** *** ** **" readOnly={!editing} />
            </div>
          </section>
